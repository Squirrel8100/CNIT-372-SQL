...
--Question 3 (Package created, Package body in question 8,9,10)
...
create or replace function Reminder
    return varchar2
is  
...
--Question 7 Member ID, Borrow ID, Item ID
create or replace procedure member_checked
(p_member_id varchar2)
as

v_borrow_id borrowing.borrowing_id%type;
v_item_id borrowing.item_id%type;
...
fetch checked_out into v_item_title, v_item_id, v_borrow_id;

while checked_out%found loop
dbms_output.put (v_item_title);
dbms_output.put (v_item_id);
dbms_output.put (v_borrow_id);
fetch checked_out into v_item_title, v_item_id, v_borrow_id;
end loop;
close checked_out;
end member_checked;
